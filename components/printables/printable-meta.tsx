import { Printable } from "@/lib/content";
import { format } from "date-fns";
import { DownloadCount } from "@/components/analytics/download-count";
import Link from "next/link";

interface PrintableMetaProps {
  printable: Printable;
}

export function PrintableMeta({ printable }: PrintableMetaProps) {
  const publishedDate = printable.publishedAt
    ? format(new Date(printable.publishedAt), "d MMMM yyyy")
    : null;

  return (
    <div className="flex flex-wrap items-center gap-4 text-sm text-text-medium">
      {printable.duration && (
        <div className="flex items-center gap-2">
          <svg
            className="w-4 h-4"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
            />
          </svg>
          <span>Διάρκεια: {printable.duration}</span>
        </div>
      )}

      {publishedDate && (
        <div className="flex items-center gap-2">
          <svg
            className="w-4 h-4"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
            />
          </svg>
          <span>{publishedDate}</span>
        </div>
      )}

      {printable.ageGroups && printable.ageGroups.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          {printable.ageGroups.map((ageGroup) => (
            <Link
              key={ageGroup._id}
              href={`/drastiriotites?age=${ageGroup.slug}`}
              className="px-3 py-1 rounded-full bg-secondary-blue/10 text-secondary-blue text-xs font-medium hover:bg-secondary-blue/20 transition"
            >
              {ageGroup.title}
            </Link>
          ))}
        </div>
      )}

      <DownloadCount
        content_slug={printable.slug}
        showIcon={true}
      />
    </div>
  );
}

